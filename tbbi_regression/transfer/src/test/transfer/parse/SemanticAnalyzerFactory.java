...
public final class SemanticAnalyzerFactory {
  ...
  static private Log LOG = LogFactory.getLog(SemanticAnalyzerFactory.class.getName());
  static ConcurrentHashMap<Integer, HiveOperation> commandType = new ConcurrentHashMap<Integer, HiveOperation>();
  static ConcurrentHashMap<Integer, HiveOperation[]> tablePartitionCommandType = new ConcurrentHashMap<Integer, HiveOperation[]>();
  ...
  static {
    ...
    tablePartitionCommandType.put(HiveParser.TOK_ALTERTABLE_PARTITIONPROPERTIES,
        new HiveOperation[] {HiveOperation.ALTERTABLE_PARTITIONPROPERTIES,
            HiveOperation.ALTERPARTITION_PARTITIONPROPERTIES });
 ...
 //   tablePartitionCommandType.put(HiveParser.TOK_ALTERPARTITION_LIFECYCLE,
 //       new HiveOperation[] {null, HiveOperation.ALTERPARTITION_LIFECYCLE});
  }
  public static HiveOperation getCommandType(ASTNode tree){
    return commandType.get(tree.getToken().getType());
  }
    ...
    if (tree.getToken() == null) {
      ...
      case HiveParser.TOK_LOAD:
        if(HiveConf.getBoolVar(conf, HiveConf.ConfVars.APSARA_MOYE_RESTRICTION)){
          throw new SemanticException(ErrorMsg.GENERIC_ERROR.getMsg("unrecognized command 'LOAD'"));
        }
        return new LoadSemanticAnalyzer(conf);
      case HiveParser.TOK_EXPORT:
        if(HiveConf.getBoolVar(conf, HiveConf.ConfVars.APSARA_MOYE_RESTRICTION)){
          throw new SemanticException(ErrorMsg.GENERIC_SEMANTIC_ERROR.getMsg("unrecognized command 'EXPORT'"));
        }
        return new ExportSemanticAnalyzer(conf);
      case HiveParser.TOK_IMPORT:
        if(HiveConf.getBoolVar(conf, HiveConf.ConfVars.APSARA_MOYE_RESTRICTION)){
          throw new SemanticException(ErrorMsg.GENERIC_SEMANTIC_ERROR.getMsg("unrecognized command 'IMPORT'"));
        }
      ...
      case HiveParser.TOK_DROPFUNCTION:
    		System.out.println("DDLSemanticAnalyzer");
